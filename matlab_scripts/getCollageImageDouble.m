function y = getCollageImageDouble(brain_vol, segmentationVolumeA, regionNumberA, segmentationVolumeB, regionNumberB, padding)
%Con esta función se obtiene el collage de la imagen
    ...
    counter=1;
    for i=1:k
        lut = double(squeeze(segmentationVolumeA(:,:,i)));
        slice = double(squeeze(brain_vol(:,:,i)));
	lutB=segmentationVolumeB(:,:,i);
        if (sum(sum(regionCrop(regionNumberA,slice,lut,h,w)))>0)
	    [slice_crop, r1, r2, c1, c2]=regionCropSaveDims(regionNumberA,slice,lut,h,w);
	    slice_crop_B = regionSegSlice(regionNumberB, slice(r1:r2, c1:c2), lutB(r1:r2, c1:c2), 1+r2-r1, 1+c2-c1);
	    slice_crop_dims=size(slice_crop);
            region_slices{counter}=insertMatrix(zeros(slice_crop_dims(1)+2*padding, slice_crop_dims(2)+2*padding), slice_crop_B);
